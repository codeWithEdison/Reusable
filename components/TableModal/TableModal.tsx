...
const TableModal: React.FC<ModalProps> = (props) => {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed top-0 left-0 w-screen h-full flex items-center justify-center bg-black bg-opacity-50 pt-32 px-4 ">
          <div ref={modalRef} className="bg-white w-full h-full  py-4 rounded-t-xl animate__animated animate__fadeInUp animate__faster ">
            ...
            <div className="flex flex-col px-6  py-2 ">
              ...
              <input
                ...
                <button className=" flex  items-center justify-center rounded-lg px-8 bg-green-600 w-1/12 ml-4 text-white text-xl" onClick={exportToCSV}><FaFileExcel className="" />Export</button>
              </div>
                      ...
                      {filteredData.map((row, rowIndex) => (
                        <tr key={rowIndex} className="border-t">
                          ...
                          {tableHeaders.map((header, cellIndex) => (
                            <td key={cellIndex} className="px-4 py-2">{row[header.toLowerCase().replace(/\s+/g, '_')]}</td>
                          ))}
                        </tr>
                      ))}
                    </tbody>
